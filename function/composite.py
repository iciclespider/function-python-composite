...
class Extras:
    ...
    def __getitem__(self, key):
        return ExtraResources(self._composite, key)
...
class ExtraResources:
    ...
    def __len__(self):
        len(self._resources.items)

    def __getitem__(self, key):
        if key >= len(self._resources.items):
            return ExtraResource(fnv1.Resource())
        return ExtraResource(self._resources.items[key])

    def __len__(self):
        return len(self._resources.items)

    def __iter__(self):
        for resource in self._resources.items:
            yield ExtraResource(resource)
